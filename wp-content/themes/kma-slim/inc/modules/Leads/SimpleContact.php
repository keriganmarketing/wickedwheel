...
class SimpleContact extends Leads
{
    public function __construct ()
    {
        ...
        parent::set('adminEmail', 'info@thewickedwheel.com');
    }

    protected function showForm()
    {
        $form = file_get_contents(locate_template('template-parts/forms/contact-form.php'));
        $form = str_replace('{{user-agent}}', isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '', $form);
		$form = str_replace('{{ip-address}}', parent::getIP(), $form);
        $form = str_replace('{{referrer}}', isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '', $form);
