        ...
        <div class="container is-fluid">
            <div class="content">
                <?php
                    ...
                    if(!get_option('instagram_token') && !isset($_GET['code'])){ ?>
                        
                    <h4 id="authorize-headline" style="display: block;" class="wp-heading-inline">Authorize the App to continue</h4>
                    <div id="status"></div>
                    <p class="submit" id="authorize">
                        <button onclick="checkStatus()"
                            class="button is-primary" 
                            ><?php _e('Authorize App') ?></button>
                    </p>    
                <?php } ?>
                    <hr>
                    ...
                    <form enctype="multipart/form-data" name="instagram_settings" id="instagram_settings" method="post"
                        ...
                        <p class="submit">
                            ...
                            <a onclick="checkStatus();"
                                class="button is-primary" 
                                ><?php _e('Renew Authorization') ?></a>
            ...
            for(let i = 0; i < response.data.length; i++){
                jQuery("#accountoptions").append('<div class="column is-3" >' +
                    '<div class="card" >' +
                        ...
                        '<button class="button is-primary" onclick="chooseCompany(\''+response.data[i].id+'\',\''+response.data[i].access_token+'\',\''+longLivedUserToken+'\')" class="card-footer-item">Select Company</button>' +
                    '</div>' +
                '</div>');
            }
        }); 
...
function chooseCompany(id, pagetoken, usertoken){
    ...
    jQuery.ajax({
        ...
        if(response.instagram_business_account){
            ...
            // getPermaToken(usertoken, response.id)
            ...
            jQuery("#error").html('<div class="notification is-danger mb-5">' +
                ...
                '<p><strong>Error Code '+response.error.code+':</strong> ' + response.error.message + '</p>' +
            '</div>');
        }
    });
...
function chooseCompanyOld(id, pagetoken, usertoken){
    FB.api('/' + id + '?fields=instagram_business_account&access_token=' + pagetoken, function (response) {
        console.log(response)
        if(response.instagram_business_account){
            document.getElementById('instagramid').value = response.instagram_business_account.id;
            // getPermaToken(usertoken, response.id)
...
//2174657569497762|1Z4klT-FAQrzEJTgnbGpOsIEwQc

function getPermaToken(token, user){
    let urlParams = "token=" + token + '&user=' + user
    //console.log(urlParams)
    //console.log('attempting perma token exchange')
    jQuery.ajax({
        url: "/wp-json/kerigansolutions/v1/permatoken?" + urlParams
    }).done(function(response) {
        console.log(response);
        document.getElementById('instagramtoken').value = response.authResponse.accessToken;
    });
}
